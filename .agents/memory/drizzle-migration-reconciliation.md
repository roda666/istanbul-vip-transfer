...
**Why:** Drizzle stops on an already-existing table, index, or constraint; a migration journal timestamp earlier than a database's last applied timestamp can also prevent the intended SQL from being selected. Generated entries can inherit such an older timestamp in this reconciled workspace. Both cases leave code and the migration ledger disagreeing about the schema.
