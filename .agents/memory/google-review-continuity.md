...
The public homepage may combine selected-location Google reviews with visible manual real-customer reviews that are explicitly reviewed or carry the legacy verified-source marker. Keep original review text unchanged across locales. Deduplicate by external ID or normalized author+text, preferring the Google row when a later sync matches a manual row.

**Why:** An OAuth repair issue must not turn verified historical customer reviews into fabricated fallback content or make the homepage unstable. Hiding all verified reviews on a refresh failure also creates an unnecessary public outage. A successful OAuth callback and `business.manage` scope can coexist with a zero provider quota. The account owner explicitly requires deactivation and reconnection to remain separate, user-controlled actions. Approved real manual reviews must remain visible without being relabeled or rewritten as Google content.

**How to apply:** Google-sourced public rows require a connected profile, selected account/location, visibility, and Google provenance; they do not require the sync-enabled flag. Manual rows require visibility plus explicit review or the non-user-editable legacy verification marker. Never show drafts, hidden rows, test fixtures, or unreviewed ordinary manual rows. Successful leased syncs invalidate every localized homepage only after provider and database work completes. When account listing returns a zero quota, surface the provider gate and request/confirm Google quota access before retrying. Gate only passive-to-active transitions on account/location selection; keep active-to-passive available and reconnect explicitly initiated.