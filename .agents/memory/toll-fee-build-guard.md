...
- Same substring-collision class hit Dutch ("tol" inside tolk/tolerant/tolereren/tolvrij), Russian (плат-/сбор- word families beyond the geo/fee split above), Italian ("costo" inside "costola"), Spanish ("cargo" inside "cargador"), and Arabic ("رسوم" inside "مرسوم") — same fix pattern (exact-form/negative-lookahead regex instead of the generic wildcard-suffix `tokenRe`). Logic now lives in a shared, side-effect-free module (`scripts/lib/toll-fee-rules.mjs`) so the production checker and its self-test both exercise the exact same rules — write new language fixes there, not in either consumer. Genuine homonym ambiguity (French `frais`, Spanish `cargo` in its job/position sense) is a different, unfixed bug class — don't conflate the two when triaging a new false positive.
