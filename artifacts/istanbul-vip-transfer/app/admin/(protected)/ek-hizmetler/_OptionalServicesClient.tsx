...
function ServiceForm({ form, editing, saving, onChange, onClose, onSave }: { form: FormValues; editing: OptionalService | null; saving: boolean; onChange: <K extends keyof FormValues>(key: K, value: FormValues[K]) => void; onClose: () => void; onSave: () => void }) {
  const scopeText = form.serviceTypeScope.join(', ');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-3 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="optional-service-title">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:p-6">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={onClose} className="min-h-10 min-w-10 rounded p-2 text-slate-500 hover:bg-slate-100" aria-label="Kapat"><X size={20} /></button>
        </div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Hizmet türü kapsamı"><input value={scopeText} placeholder="AIRPORT_TRANSFER, INTERCITY" onChange={(e) => onChange('serviceTypeScope', e.target.value.split(',').map((item) => item.trim().toUpperCase()).filter(Boolean))} /><span className="mt-1 block text-xs font-normal text-slate-500">Virgülle ayırın; boş kapsam public formda görünmez.</span></Field>
          ...
          <div className="space-y-3 pt-2 sm:col-span-2">
            <label className="flex min-h-10 items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.active} onChange={(e) => onChange('active', e.target.checked)} className="h-4 w-4" />Aktif</label>
            <label className="flex min-h-10 items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.includedInTransfer} onChange={(e) => onChange('includedInTransfer', e.target.checked)} className="h-4 w-4" />Transfer fiyatına dahil (müşteriye listelenmez)</label>
            <label className="flex min-h-10 items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.customerVisible} onChange={(e) => onChange('customerVisible', e.target.checked)} className="h-4 w-4" />Müşteriye görünür (yalnız ayrı ücretli hizmetlerde)</label>
          </div>
        </div>
