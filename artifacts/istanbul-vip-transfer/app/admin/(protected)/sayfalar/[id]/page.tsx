...
export const metadata: Metadata = { title: 'Sayfayı Düzenle | Admin', robots: { index: false } };

export default async function EditSayfaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!item || item.contentType !== 'PAGE') notFound();
  ...
  return (
    <div style={{ padding: '28px 24px' }}>
      ...
      <AdminPageHeader title="Sayfayı Düzenle" description={item.title} />
      {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
      <ContentForm mode="edit" contentType="PAGE" initialData={dto as any} backUrl="/admin/sayfalar" />
    </div>
  );
}
