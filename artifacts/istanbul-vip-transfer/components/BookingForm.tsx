                ...
                <section className="mb-6 rounded-xl border border-slate-200 bg-white p-5" aria-labelledby="optional-services-heading">
                  ...
                  <div className="space-y-3">
                    {optionalServices.map((service) => {
                      ...
                      return <label key={service.id} className={`flex min-h-11 items-center justify-between gap-3 rounded-xl border p-3 text-sm text-slate-700 transition-colors ${quantity > 0 ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-200' : 'border-slate-200 bg-white hover:border-blue-300'}`}>
                        <span className="flex min-w-0 items-center gap-2"><input aria-label={`${service.name} seç`} type="checkbox" checked={quantity > 0} onChange={(event) => setSelectedOptionalServices((current) => ({ ...current, [service.id]: event.target.checked ? 1 : 0 }))} className="h-5 w-5 shrink-0 accent-blue-600" />{quantity > 0 && <span aria-hidden="true" className="font-bold text-blue-600">✓</span>}<span><span className="font-semibold">{service.name}</span>{service.shortDescription && <span className="ml-1 text-xs text-slate-500">— {service.shortDescription}</span>} <span className="text-xs text-slate-500">({amount}{quantity > 1 ? ` × ${quantity}` : ''})</span></span></span>
