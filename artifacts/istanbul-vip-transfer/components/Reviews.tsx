...
// dates are not available and are omitted.
const reviews = [
  {
    name: 'Ahmet Kaya',
    rating: 5,
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 mb-5 px-4 py-2 rounded-full"
            ...
            <GoogleMark size={16} />
            <span className="text-xs tracking-[0.2em] uppercase" style={{ color: '#AAAAAA', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-7 mb-14">
          {reviews.map((review, i) => (
            <motion.div
              ...
              style={{ background: 'linear-gradient(160deg, #161616 0%, #1A1A1A 100%)', border: '1px solid rgba(201,168,76,0.15)' }}
            ...
            >
              ...
              <div className="mb-5"><Quote size={28} style={{ color: 'rgba(201,168,76,0.25)' }} /></div>
              <div className="flex items-center gap-1 mb-5" aria-label={`${review.rating} yıldız`} data-testid={`review-stars-${i}`}>
                {Array.from({ length: review.rating }).map((_, si) => (
                  <Star key={si} size={14} fill="#C9A84C" stroke="none" />
                ))}
              </div>
              <p className="text-sm leading-relaxed flex-1 mb-6" style={{ color: '#AAAAAA', fontFamily: 'Inter, sans-serif', fontStyle: 'italic' }}>
                &ldquo;{review.text}&rdquo;
              </p>
              <div className="flex items-center justify-between pt-5" style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-semibold" style={{ color: '#E5E5E5', fontFamily: 'Inter, sans-serif' }}>
                    {review.name}
                  </p>
                </div>
