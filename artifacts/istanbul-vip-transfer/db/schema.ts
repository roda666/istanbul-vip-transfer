...
/**
 * Immutable SMTP evidence for every transactional send attempt. This keeps the
 * provider's acceptance/rejection response visible to the team without ever
 * storing SMTP credentials or message bodies.
 */
export const emailDeliveryAttempts = pgTable(
  ...
  (table) => [
    ...
    index('email_delivery_attempts_request_reference_idx').on(table.requestReference),
  ],
);
...
/**
 * Singleton envelope-encrypted data key for SMTP passwords.
 *
 * The random data key is generated by the application and stored only after
 * being encrypted with a key derived from an existing Replit-managed app
 * secret. It is intentionally separate from email_settings so a database
 * backup alone cannot recover SMTP passwords.
 */
export const emailEncryptionKeys = pgTable('email_encryption_keys', {
  id:         integer('id').primaryKey().default(1),
