...
const specs = [
  {
    key: 'hero',
    fileName: 'vip-taksi-karsilastirma-hero.jpg',
    alt: 'Havalimanında bekleyen VIP taksi/transfer aracı',
    prompt: 'Photorealistic three-quarter front view of a black premium sedan-style vehicle parked in a clean airport pickup zone at soft daylight, orderly and calm atmosphere. Editorial automotive photography, natural colour grading, no text, no logos, no brand emblems, no visible number plate, no readable signage.',
  },
  {
    key: 'inline1',
    fileName: 'havalimani-taksi-duragi.jpg',
    alt: 'Havalimanı taksi durağında bekleyen yolcular',
    prompt: 'Photorealistic wide shot of a busy airport taxi rank area with several yellow taxis queued under soft daylight, travelers with luggage waiting at a distance, orderly but bustling atmosphere. Documentary travel photography, natural colours, no text, no logos, no readable signage, no visible faces in close focus, no number plates.',
  },
  {
    key: 'inline2',
    fileName: 'sabit-fiyat-rezervasyon.jpg',
    alt: 'Önceden rezervasyonla planlanan VIP transfer aracı',
    prompt: 'Photorealistic close-up three-quarter view of a clean car dashboard area (no driver visible) with soft ambient light, minimal and calm interior, no visible screens with text. Editorial automotive interior photography, natural colours, no text, no logos, no brand marks.',
  },
...
async function generateImages() {
  ...
  for (const spec of specs) {
    ...
    await upload(
      webp,
      `ai-images/blog/${slug}/${spec.fileName.replace(/\.jpg$/i, '.webp')}`,
    );
...
async function placeImagesAndQueue() {
  const byKey = Object.fromEntries(
    specs.map(spec => [spec.key, { ...spec, url: imageUrl(spec) }]),
  );
  const result = await sql.begin(async tx => {
    ...
    if (
      (before.body.match(/\[\[IMAGE:inline-1\]\]/g) ?? []).length !== 1 ||
      (before.body.match(/\[\[IMAGE:inline-2\]\]/g) ?? []).length !== 1
    ) throw new Error('Image placeholders missing or duplicated');

    const placedBody = before.body
      .replace(
        '[[IMAGE:inline-1]]',
        `![${byKey.inline1.alt}](${byKey.inline1.url})`,
      )
      .replace(
        '[[IMAGE:inline-2]]',
        `![${byKey.inline2.alt}](${byKey.inline2.url})`,
      );
    await tx`
      UPDATE content SET
        body=${placedBody},
    ...
    const [job] = await tx`
      INSERT INTO translation_jobs (
        entity_type, entity_id, status, force,
    ...
    await tx`
      ...
      SELECT ${job.id}, code, 'QUEUED', 0
      FROM unnest(${languages}::text[]) AS code`;

    const [after] = await tx`
      SELECT slug,status,published_at,updated_at,
             hero_image,hero_image_alt,og_image,body
    ...
    if (
      after.body.includes('[[IMAGE:inline-1]]') ||
      after.body.includes('[[IMAGE:inline-2]]')
